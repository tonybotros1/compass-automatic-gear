...
const _minimumTableWidth = 1180.0;

enum _ExpenseScope { all, cars, general }
...
class _ExpensesSectionState extends State<ExpensesSection> {
  ...
  String _query = '';
  _ExpenseScope _scope = _ExpenseScope.all;
  int _page = 0;
  ...
  Widget build(BuildContext context) {
    return GetX<CarTradingDashboardController>(
      builder: (controller) {
        final items = _visibleItems(controller.allGeneralExpenses);
          ...
          child: Column(
            ...
            children: [
              _ExpensesToolbar(
                ...
                onNew: () => _openNewExpense(controller),
              ),
              ...
              Expanded(
                child: _ExpensesPanel(
                  ...
                  onEdit: (item) => _openEditExpense(controller, item),
                  onDelete: (item) => _deleteExpense(controller, item),
                ),
              ),
            ],
          ),
  ...
  List<GeneralExpensesModel> _visibleItems(List<GeneralExpensesModel> source) {
    final query = _query.trim().toLowerCase();
    final result = source
        .where((item) {
          final hasCar = item.tradeId.trim().isNotEmpty;
          if (_scope == _ExpenseScope.cars && !hasCar) return false;
          if (_scope == _ExpenseScope.general && hasCar) return false;
  ...
  void _openNewExpense(CarTradingDashboardController controller) {
    ...
    itemDialog(
      ...
      canEdit: true,
      onPressed: () {
        controller.addNewCapitalOrOutstandingOrGeneralExpenses(
          'general_expenses',
        );
      },
    );
  ...
  void _openEditExpense(
    ...
    GeneralExpensesModel item,
  ) {
    controller.item.text = item.item;
    controller.itemId.value = item.itemId;
    controller.tradingCar.text = _carTextWithTrim(item);
    controller.tradingCarId.value = item.tradeId;
  ...
  void _deleteExpense(
    ...
    alertDialog(
      context: context,
      content: 'This expense will be deleted permanently.',
      onPressed: () {
...
class _ExpensesToolbar extends StatelessWidget {
  const _ExpensesToolbar({
  ...
  final _ExpenseScope scope;
  final bool isRefreshing;
  final ValueChanged<String> onSearchChanged;
  final ValueChanged<_ExpenseScope> onScopeChanged;
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        final search = _ExpensesSearchField(
        ...
        final actions = Row(
          ...
          children: [
            _ExpenseScopePicker(value: scope, onChanged: onScopeChanged),
            const SizedBox(width: 8),
            _EntriesPill(count: entryCount),
            const SizedBox(width: 8),
            _RefreshButton(isRefreshing: isRefreshing, onPressed: onRefresh),
            const SizedBox(width: 8),
            _NewExpenseButton(isLoading: isRefreshing, onPressed: onNew),
          ],
        );
...
class _ExpensesSearchField extends StatelessWidget {
    ...
    return SizedBox(
      ...
      child: TextField(
        ...
        decoration: InputDecoration(
          hintText: 'Search item, car, account, date, or comment',
...
class _ExpenseScopePicker extends StatelessWidget {
  const _ExpenseScopePicker({required this.value, required this.onChanged});

  final _ExpenseScope value;
  final ValueChanged<_ExpenseScope> onChanged;
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          _ScopeOption(
            label: 'ALL',
            selected: value == _ExpenseScope.all,
            onTap: () => onChanged(_ExpenseScope.all),
          ),
          _ScopeOption(
            label: 'CARS',
            selected: value == _ExpenseScope.cars,
            onTap: () => onChanged(_ExpenseScope.cars),
          ),
          _ScopeOption(
            label: 'GENERAL',
            selected: value == _ExpenseScope.general,
            onTap: () => onChanged(_ExpenseScope.general),
          ),
        ],
      ),
    );
...
class _NewExpenseButton extends StatelessWidget {
  const _NewExpenseButton({required this.isLoading, required this.onPressed});
...
class _ExpensesTotals extends StatelessWidget {
  const _ExpensesTotals({
    required this.paid,
...
class _ExpensesPanel extends StatelessWidget {
  ...
  final List<GeneralExpensesModel> items;
  final bool isLoading;
  final int requestedPage;
  final ScrollController horizontalController;
  final ValueChanged<int> onPageChanged;
  final ValueChanged<GeneralExpensesModel> onEdit;
  final ValueChanged<GeneralExpensesModel> onDelete;
    ...
    return Container(
      ...
      child: LayoutBuilder(
        builder: (context, constraints) {
          ...
          if (items.isEmpty) return const _ExpensesEmptyState();
...
class _ExpenseRow extends StatelessWidget {
  ...
  final GeneralExpensesModel item;
  final bool shaded;
  ...
  @override
  Widget build(BuildContext context) {
    final car = _carTextWithTrim(item);
    final net = item.receive - item.pay;
    return Container(
      height: 58,
      ...
      child: _ExpenseGrid(
        ...
        net: _MoneyText(value: net, color: _netColor(net)),
      ),
    );
  }
}
    ...
    return DecoratedBox(
      ...
      child: Row(
        children: [
          ...
          Expanded(flex: 4, child: cell(car)),
      ...
      children: [
        ...
        Expanded(
          child: Text(
            ...
            overflow: TextOverflow.ellipsis,
            style: TextStyle(
              color: muted || text.trim().isEmpty ? _muted : _text,
...
class _ExpensesPaginationFooter extends StatelessWidget {
  const _ExpensesPaginationFooter({
    required this.start,
    required this.end,
    required this.total,
    required this.page,
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Row(
        children: [
          Text(
            'Showing ${total == 0 ? 0 : start + 1}–$end of $total expenses',
            style: const TextStyle(
              color: _muted,
              fontSize: 10,
              fontWeight: FontWeight.w700,
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const Text(
              'Try changing the search or add a new expense.',
...
String _carTextWithTrim(GeneralExpensesModel item) {
  final car = item.car.trim();
  final trim = item.trim.trim();
