...
class LoanAndAdvancesTypesController extends GetxController {
  ...
  Future<void> addNewLeaveType() async {
    try {
      ...
      Uri url = Uri.parse('$backendUrl/loan_and_advances_types/add_new_loan_and_advances_type');
  ...
  Future<void> editLeaveType(String id) async {
    try {
      ...
      var accessToken = '${prefs.getString('accessToken')}';
      final refreshToken = '${await secureStorage.read(key: "refreshToken")}';
      Uri url = Uri.parse('$backendUrl/loan_and_advances_types/update_loan_and_advances_type/$id');
  ...
  Future<bool> deleteLeaveType(String typeId) async {
    try {
      ...
      var accessToken = '${prefs.getString('accessToken')}';
      final refreshToken = '${await secureStorage.read(key: "refreshToken")}';
      Uri url = Uri.parse('$backendUrl/loan_and_advances_types/delete_loan_and_advances_type/$typeId');
  ...
  Future<void> searchEngine(Map<String, dynamic> body) async {
    try {
      ...
      final refreshToken = '${await secureStorage.read(key: "refreshToken")}';
      Uri url = Uri.parse(
        '$backendUrl/loan_and_advances_types/search_engine_for_loan_and_advances_types',
      );
      final response = await http.post(
        url,
        headers: {
      ...
      if (response.statusCode == 200) {
        final decoded = _jsonObject(response.body);
        List docs = decoded['loan_and_advances_types'] ?? [];
